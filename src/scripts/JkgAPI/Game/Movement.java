...
public class Movement {
	...
	/**
	 * Checks if we can reach the specified Positionable
	 * (RSTile/RSObject/RSPlayer/RSGroundItem etc)
	 *
	 * @param toReach
	...
	/**
	 * Sets the use of custom door objects. Generally used if DPathNavigator
	 * cannot recognize a door.
	 * <p>
	...
	public static boolean walkToPrecise(Positionable posToWalk) {
		...
		// DPathNavigator will fail if it cannot find a path, including when the
		// target is an unwalkable tile, so try to find the nearest walkable
		// tile.
	...
	/**
	 * Walks to the position using either DPathNavigator for close by precision
	 * or WebWalking for greater lengths. if the target is unwalkable, will try
	 * to find nearest walkable tile. Checks if run can be toggled.
	 *
	 * @param posToWalk
	...
	/**
	 * Walks to the position using either DPathNavigator for close by precision
	 * or WebWalking for greater lengths.
	 * <p>
	 * Checks if run can be toggled.
	 *
	 * @param posToWalk
	 * @param searchWalkable
	 *            if the target is unwalkable, will try to find nearest walkable
	 *            tile.
	 * @return if successfully reached destination or not.
	 */
	...
	/**
	 * Walks to the position by straight user-made paths. Will try to handle
	 * doors. This is mostly used in areas which WebWalking has not mapped.
	 * <p>
	 * Checks if run can be toggled.
	 *
	 * @param path
