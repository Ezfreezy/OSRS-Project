...
import scripts.JkgAPI.Game.Helpers.SkillsHelper;
 ...
 * @author Laniax
 */
public class JKGAntiban extends org.tribot.api.util.ABCUtil {

    private static JKGAntiban antiban;
    ...
    // singleton
    public static JKGAntiban getUtil() {
        return antiban = antiban == null ? new JKGAntiban() : antiban;
    }
    ...
    public static void resetEatPercentage() {
        eatAtPercentage = JKGAntiban.getUtil().INT_TRACKER.NEXT_EAT_AT.next();
    }
    ...
    public static void waitDelay(boolean isCombat) {
        if (idle && idleSince > (System.currentTimeMillis() + General.random(8000, 12000))) {
            ...
            JKGAntiban.doDelayForNewObject(isCombat);
        } else {
            ...
            JKGAntiban.doDelayForSwitchObject(isCombat);
        }
    }
};